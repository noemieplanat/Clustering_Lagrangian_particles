...
from Config3 import norm, t, resamp, length_days, wtype, kernel, degreek,   n_components, copy, whiten, svd_solver,\
...
if not os.path.exists(path_save_clustering+'ClassifiedData/'):
    os.makedirs(path_save_clustering+'ClassifiedData/')
if not os.path.exists(path_save_clustering+'ClassifiedData/labels_data_2014.npy'):
    apply_to_all_data(path_save_clustering, ntest, init_year,final_year, length_days, files, start_lon,  norm, t, resamp,wtype, name_PCA_config, path_save_PCA, Dataset_path, perctest, perctrain, kernel)