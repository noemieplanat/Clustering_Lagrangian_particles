...
def extract(path, length_days, start_lon) :
    ...
    lons = lons_in_interval(start_lon, ds.lon[:,0:length_days].values)
...
def Part_1_pre_processing(Dataset_path, length_days, perctest, perctrain, norm, t, resamp,wtype, start_lon):#, Dataset_path, norm, t, resamp, length_days, wtype, kernel, degreek, lats_valid, lons_valid, lats_train, \
                          #lons_train, lats_test, lons_test,  n_components, copy, whiten, svd_solver):
...
def run_script_prediction(name, norm, t, resamp, length_days, wtype, lats, lons, lats_train, lons_train, path_save, name_kernel_pca, X_centers_train, path_kernelpca) :   
...
def apply_to_all_data(path_save_clustering, ntest, init_year,final_year, length_days, files, start_lon,  norm, t, resamp,wtype, name_PCA_config, path_save_PCA):
    centroids, labels_test, labels_valid, lats_test, lats_valid, lons_test, lons_valid, lons_train, lats_train, \
        X_reduced_test, X_Reduced_valid, temps_test, temps_train, temps_valid, sals_test, sals_train, sals_valid, X_centers_train = load_data_test_train_valid(path_save_clustering, ntest)
...
def load_data_test_train_valid(path_save_clustering, ntest, Dataset_path, length_days, perctest, perctrain, start_lon):
    ...
    return CENTROID, LABELS_TEST, LABELS_VALID, lats_test, lats_valid, lons_test, lons_valid, lons_train, lats_train, \
        X_REDUCED_TEST, X_REDUCED_VALID, temps_test, temps_train, temps_test, sals_test, sals_train, sals_valid, X_centers_train
