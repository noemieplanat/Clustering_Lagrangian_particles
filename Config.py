...
name_subset = 'Sub_samp_y_%i'%init_year +'_-_%i'%final_year+'_delta_%i' %delta_year +'_%i'%N_particles+ '_%i'+'start_lon_%i'%start_lon+'.nc'
...
n_components = 0.99999 #Either we specify the number of features that we want (n_components=int) either the total variance explainded (inf 1)
copy = True #Do not modify the given vector, rather copy it
svd_solver = 'auto' 
whiten = False
kernel = 'cosine' # if None, ran during the Clustering, else, prior to it. ‘linear’, ‘poly’, ‘rbf’, ‘sigmoid’, ‘cosine’, ‘precomputed’
degreek = False # if use poly kernel

norm = True # if normalize
t = True # translation
resamp = False # resampling
wtype = False # Type of weight for geographic correction. None if no weight. Otherwise cos or sqrt-cos
...
verbose = 0 
sample_weight = None
n_split = 30  # number of iterations for convergence
