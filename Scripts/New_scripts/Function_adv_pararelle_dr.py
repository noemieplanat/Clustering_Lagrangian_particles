...
def run_adv_journalier_sensitivity(tup):
    ...
    for i in range(len(yrs)-4):
        ...
        repeatdt = delta(days=dr) # release a new set of particles every day
        ...
        pset.execute(kernels, runtime=delta(days=90), dt=delta(minutes = dt), output_file=output_file, verbose_progress=True)
        pset.repeatdt = None
