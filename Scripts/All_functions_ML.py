...
def random_sample(N_particles, ds):
    ...
    ds_samp = ds.sel(traj=xr.DataArray(sample)) # select the random particles
    return ds_samp
...
def Part_1_pre_processing(norm, t, resamp, length_days, wtype, kernel, degreek, lats_valid, lons_valid, lats_train, lons_train, lats_test, lons_test,  n_components, copy, whiten, svd_solver):
    print('Pre_processing')
    # PRE-PROCESSING
    w, X_lats_lons_train = Part_1_pre_processing_one_sort(t, lats_train, lons_train, resamp, length_days, wtype, norm)
...
def Part_2_PCA(X_lats_lons_valid, X_lats_lons_test, X_lats_lons_train, w, w_valid, w_test,  n_components, copy, whiten, svd_solver, kernel, degreek):
...
def PCA_Non_Kernelized(Xlat_lon, n_components, copy, whiten, svd_solver, w):
    return PCA_Kernelized(Xlat_lon, n_components, copy, whiten, svd_solver, w, kernel=None, degreek=0)
...
def Save_PCA(path, name_PCA_config,X_reduced_train, X_reduced_valid, X_reduced_test, a, l, dc ):
...
def Part_2_Load_PCA( name, norm, t, resamp, length_days, wtype, n_clusters, kernel, lats_val, lons_val,lats_train, lons_train, path_save, name_kernel_pca, lats_test,lon_test, path_kernelpca) :   
...
def Part_3_kmeans_clustering(X_reduced_train, X_reduced_valid, X_reduced_test, n_split, n_clusters, init, nmb_initialisations, max_iter, tol, algorithm, verbose, sample_weight):    
    # K-MEANS CLUSTERING
    print('Kmeans')
...
def Part_3_save_clustering(lats_test, lons_test, lats_valid, lons_valid, temps_valid, sals_valid, sals_test, temps_test, lats_train, lons_train, temps_train, sals_train, ntest, path_save_clustering, labels_valid, labels_test, X_reduced_valid, X_reduced_test, centroids, a_temp,X_centers_train):
...
def Part_4_prediction(X_lats_lons, X_lats_lons_train, Alphas, X_centers_train) :   
...
def run_script_prediction(name, norm, t, resamp, length_days, wtype, n_clusters, kernel, lats, lons, lats_train, lons_train, path_save, name_kernel_pca, X_centers_train, path_kernelpca) :   
    ...
    return Part_4_prediction(X_lats_lons, X_lats_lons_train, Alphas, X_centers_train)

def apply_to_all_data(files, init_year, final_year, path_save_PCA,X_centers_train, lats_train, lons_train, name_PCA_config, Dataset_path, length_days, path_save_clustering, n_clusters, ntest, wtype, kernel, norm, t, resamp):
    for yr in range(init_year,final_year):
        print('Year', yr)
        lats_all,lons_all, temps_all, sals_all, date_all = extract_all_per_year(Dataset_path, yr, length_days, files)
        labels_all = []
        for sep in range(0,lats_all.shape[0],500) :
            ...
            labels, X_reduced =  run_script_prediction(ntest, norm, t, resamp, length_days, wtype, n_clusters,
                                            kernel, lats_all_l, lons_all_l, lats_train, lons_train, path_save_clustering,
                                            name_PCA_config, X_centers_train, path_save_PCA)
            labels_all.extend(labels)
        np.save(path_save_clustering+'ClassifiedData/'+'labels_data_%i'%yr, labels_all)
        np.save(path_save_clustering+'ClassifiedData/'+'time_data_%i'%yr, date_all)
        np.save(path_save_clustering+'ClassifiedData/'+'lons_data_%i'%yr, lons_all)
        np.save(path_save_clustering+'ClassifiedData/'+'lats_data_%i'%yr, lats_all)
