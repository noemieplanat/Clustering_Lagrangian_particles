...
def run_adv_journalier_sensitivity(tup):
    ...
    for i in range(len(yrs)-4):
        ...
        if type(dr)==int: 
            ...
            repeatdt = delta(days=dr) # release a new set of particles every week
        ...
        pset.execute(kernels, runtime=delta(days=2*365), dt=delta(minutes = dt), output_file=output_file, verbose_progress=True)
        ...
        #pset.execute(kernels, runtime=delta(days=4*365), dt=delta(minutes = dt), output_file=output_file, verbose_progress=True)
